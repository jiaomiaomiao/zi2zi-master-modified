...
from .ops_multi_gpus import conv2d, deconv2d, lrelu, fc, batch_norm, init_embedding_dictionary,init_embedding_weights, conditional_instance_norm, weight_norm
...
class UNet(object):
    ...
    def build_model(self, is_training=True, inst_norm=False,current_gpu_id=-1):
        ...
        # current_summary_handle = SummaryHandle(d_merged=d_merged_summary,
        ...
        # summaryHandleList.append(current_summary_handle)


        # those operations will be shared, so we need
        # to make them visible globally
        # setattr(self, "input_handle", inputHandleList)
        #setattr(self, "loss_handle", lossHandleList)
        setattr(self, "eval_handle", eval_handle)
        #setattr(self, "summary_handle", summaryHandleList)
    ...
    def summary_finalization(self,loss_list):
        ...
        d_loss_final = tf.divide(tf.add_n(d_loss_final),
                                 ...
                                 name='d_loss_final')
        real_category_loss_summary = tf.summary.scalar("real_category_loss", real_category_loss_final)
        fake_category_loss_summary = tf.summary.scalar("fake_category_loss", fake_category_loss_final)
        ...
        ebdd_weight_dynamic_checker_final = tf.divide(tf.add_n(ebdd_weight_dynamic_checker_final),
                                                      tf.ones(shape=ebdd_weight_dynamic_checker_final[0].shape),
    ...
    def train_procedures(self):
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
                ...
                for ii in xrange(len(available_gpu_list)):
                    with tf.device(available_gpu_list[ii]):
                        with tf.name_scope('tower_%d' % (ii)) as scope:
                            ...
                            # for var_traveller in all_vars:
                            #     print ("%s @ %s" % (var_traveller.device, var_traveller.name))
                            #     if var_traveller.device.find('CPU:0')==-1:
                            #         print (var_traveller.name)


                            #_,loss_handle, _,_,_ = self.retrieve_handles()
            ...
            print("Initialization completed, and training started.")
            ...
            for ei in range(self.epoch):
                ...
                for bid, batch in enumerate(train_batch_iter):
                    ...
                    print("Time:%s, Epoch:%d, Itr:%d, ItrDuration:%.2fss, FullDuration:%.2fhs;" % (current_time,ei,self.counter,passed_itr,passed_full/3600))
                    ...
                    if self.counter % self.sample_steps == 0:
                        ...
                        # check for train set
                        merged_pair_train = self.check_train_model(batch_images=batch_images[0:batch_size_real,:,:,:],
                        ...
                        if self.freeze_ebdd_weights==0:
                            print(self.print_separater)
                            weights_bar_img_path = self.weight_plot_and_save(weight_to_plot=summary_handle.ebdd_weight_dynamic_checker_final.eval(session=self.sess),epoch=ei)
                            weight_bar_img = self.png_read(weights_bar_img_path)
                            weight_org_bar_summary_out = self.sess.run(summary_handle.ebdd_weights_dynamic_bar,feed_dict={summary_handle.ebdd_weights_dynamic_bar_placeholder: weight_bar_img})
