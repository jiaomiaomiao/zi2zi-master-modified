...
def conv2d(x, output_filters, kh=5, kw=5, sh=2, sw=2, stddev=0.02, scope="conv2d"):
    with tf.variable_scope(scope):
        ...
        W = variable_creation_on_cpu('W',shape=[kh, kw, shape[-1], output_filters],initializer=tf.truncated_normal_initializer(stddev=stddev))
        ...
        biases = variable_creation_on_cpu('b',shape=[output_filters],initializer=tf.constant_initializer(0.0))
...
def deconv2d(x, output_shape, kh=5, kw=5, sh=2, sw=2, stddev=0.02, scope="deconv2d"):
    with tf.variable_scope(scope):
        ...
        W = variable_creation_on_cpu('W',shape=[kh, kw, output_shape[-1], input_shape[-1]],initializer=tf.random_normal_initializer(stddev=stddev))
        ...
        biases = variable_creation_on_cpu('b',shape=[output_shape[-1]],initializer=tf.constant_initializer(0.0))
...
def fc(x, output_size, stddev=0.02, scope="fc"):
    with tf.variable_scope(scope):
        ...
        W = variable_creation_on_cpu("W", shape=[shape[1], output_size],initializer=tf.random_normal_initializer(stddev=stddev))
        b = variable_creation_on_cpu("b", shape=[output_size],initializer=tf.constant_initializer(0.0))
...
def init_embedding_dictionary(size, dimension, stddev=0.01, scope="generator"):
    with tf.variable_scope(scope):
        ...
        return variable_creation_on_cpu("gen_ebdd_dictionary",shape=[size, dimension],initializer=tf.random_normal_initializer(stddev=stddev))
...
def init_embedding_weights(size, stddev=1, scope="generator", name='tmp'):
    with tf.variable_scope(scope):
        ...
        #                               tf.random_normal_initializer(stddev=stddev))
        init_weight = variable_creation_on_cpu(name,shape=size,initializer=tf.random_normal_initializer(stddev=stddev))
...
def variable_creation_on_cpu(name, shape, initializer):
